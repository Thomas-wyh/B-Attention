...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, input_dim, embed_dim, num_heads, max_ctx_k=120, temperature=1.0):
        ...
        # self.norm = nn.LayerNorm(max_ctx_k)
    ...
    def forward(self, x, mask=None, return_attention=False):
        ...
        #self_attn = self_attn / math.sqrt(d_k) # To Confirm: where to put this???
        ...
        # adj_mat = F.normalize(adj_mat, p=2, dim=-1) # To Confirm: do we need this ???
        # adj_mat = self.norm(adj_mat)
        ctx_qk = self.ctx_qk_proj(adj_mat) # [Batch, Head, SeqLen, 2*SeqLen]
        ctx_q, ctx_k = ctx_qk.chunk(2, dim=-1)
        ctx_q = F.normalize(ctx_q, p=2, dim=-1) # To Confirm: do we need this ???
        ctx_k = F.normalize(ctx_k, p=2, dim=-1) # To Confirm: do we need this ???
        ...
        # attn = self.norm(attn)
        # attn_min, _ = attn.min(dim=-1, keepdim=True)
        # attn_min = attn_min.view(-1,1).repeat(1,attn.shape[-1]).view(attn.shape)
        # attn = attn - attn_min
